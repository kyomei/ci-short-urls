...
class User extends CI_Controller {
		...
		if($this->form_validation->run() == FALSE) {
			...
			if($res) {
				foreach($res as $result) {
					if (password_verify($dataLogin['senha'], $this->senha)) {
						$data['error'] = null;
	...
	public function Register()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|min_lenth[1]|valid_email|trim');
		$this->form_validation->set_rules('senha', 'Senha', 'required|min_length[6]|trim');
		if($this->form_validation->run() == FALSE) {
			$data['error'] = validation_errors();
		} else {
	...
	public function UpdatePassw()
	{
		$data['success'] = null;
		$data['error'] = null;
		$this->form_validation->set_rules('senha', 'Senha', 'required|min_lenth[6]|trim');
		if($this->form_validation->run() == FALSE) {
			$data['error'] = validation_errors();
		} else {
	...
	public function URLs()
	{
		...
		$this->load->view('minhas_urls', $data);
	}
}
